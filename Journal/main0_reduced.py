...
def get_compare(T, d, N, M, true_beta, true_gamma, true_const, x_all, G_all, prob_all, locini, locother, newagent_time,
    ...
    quota1 = None
    quota2 = torch.tensor(np.array((pub_num_all/15/365*30/4/5).numpy().round(0),dtype=np.int64),dtype=torch.int64)
    print(quota2)
    propose = 1
    largevalue = 1e8
    print_process = True
...
for loop in range(looptimes):
    ...
    with open('result/result'+str(simu_num)+'_reduced.pkl', 'wb') as f:
