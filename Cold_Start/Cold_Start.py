...
def get_compare(T, d, N, M, true_beta, true_gamma, true_const, x_all, G_all, prob_all, locini, locother, newagent_time,
    ...
    quota1 = None
    quota2 = torch.tensor([5,5,5,5], dtype=torch.int64)
    propose = 1
    largevalue = 1e5
    print_process = False
    def get_error(x):
...
def temp_procedure(T):
    ...
    for loop in range(looptimes):
        print(f'\r|{"█" * int(50 * (loop + 1) / looptimes)}{"-" * (50 - int(50 * (loop + 1) / looptimes))}| {(loop + 1) / looptimes * 100:.2f}%',
            end='', flush=True)
        ...
        true_const = torch.tensor([1.0])

        x_all = torch.rand(total_agent_num, d) * 2
        G_all = torch.tensor([[3,2.75,1,0.5]])
        G_all = G_all[:,torch.randperm(4)]
        prob_all = torch.ones(total_agent_num)
        ...
        outthis = [out_IUCB,out_IUCB_c,out_IUCB_r,out_IUCB_c_r]
...
for TT in [100,1000]:
    print(TT)
    t11=time.time()
    tempout=temp_procedure(TT)
    outsave.append(tempout)
    t22=time.time()
    print(t22-t11)

    with open('result/coldstart.pkl', 'wb') as f:
        pickle.dump(outsave, f)
